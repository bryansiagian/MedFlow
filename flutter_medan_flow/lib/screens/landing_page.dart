...
class _LandingPageState extends State<LandingPage>
  ...
  // ── Prediksi rute ────────────────────────────────────────────
  String _routeState = 'idle'; // 'idle' | 'loading' | 'result'
  LatLng? _destPoint;
    ...
    OnboardingOverlay.show(
      ...
      steps: [
        ...
        OnboardingStep(
          ...
          description:
              'Pantau suhu dan kondisi cuaca langsung dari halaman utama.',
          padding: const EdgeInsets.all(8),
        ),
        OnboardingStep(
          targetKey: _keySearch,
          icon: OnboardingIcon.route,
          title: 'Cari Rute Pintar',
          description:
              'Ketik tujuan atau geser pin untuk prediksi waktu tempuh & kondisi lalu lintas.',
          padding: const EdgeInsets.all(6),
        ),
        OnboardingStep(
          targetKey: _keyAngkot,
  ...
  Future<void> _searchGoogle(String query) async {
    ...
    debugPrint('🔍 Searching Nominatim: $query');

    final uri = Uri.parse('https://nominatim.openstreetmap.org/search').replace(
      queryParameters: {
        'q': '$query, Medan, Sumatera Utara',
        'format': 'json',
        'limit': '6',
        'countrycodes': 'id',
        'accept-language': 'id',
    ...
    try {
      final res = await http.get(
        uri,
        headers: {'User-Agent': 'MedanFlow/1.0 (flutter app)'},
      );

      debugPrint('📡 Nominatim status: ${res.statusCode}');

      if (res.statusCode == 200) {
        final data = jsonDecode(res.body) as List;
        debugPrint('📍 Results: ${data.length}');

        final List<Map<String, dynamic>> results = data.map((p) {
          // Sederhanakan display_name agar tidak terlalu panjang
  ...
  Future<void> _selectSuggestion(String label) async {
    // Cari index manual, hindari firstWhere orElse type issue
    ...
    final fullLabel = match['fullLabel'] as String? ?? label;
    ...
    if (lat != null && lng != null) {
      final c = await _mapController.future;
      c.animateCamera(CameraUpdate.newLatLngZoom(LatLng(lat, lng), 14.5));
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ...
        c.animateCamera(
          CameraUpdate.newLatLngZoom(
            LatLng(
              (_userPosition!.latitude + dest.latitude) / 2,
              (_userPosition!.longitude + dest.longitude) / 2,
            ),
            13.0,
          ),
        );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Gagal menganalisis rute. Cek koneksi.'),
  ...
  // Angkot
  Future<void> _fetchAngkotRekomendasi(String tujuanLabel) async {
    if (tujuanLabel.isEmpty) return;
    ...
    try {
      ...
      final Map<String, String> keywordMap = {
        ...
        'sun plaza': 'Sun Plaza',
        'plaza medan': 'Plaza Medan Fair',
        'pinang baris': 'Pinang Baris',
        'padang bulan': 'Padang Bulan',
        'titi kuning': 'Titi Kuning',
        'perumnas mandala': 'Perumnas Mandala',
        'simpang pos': 'Simp. Pos',
        'terminal amplas': 'Terminal Amplas',
  ...
  Widget _buildWeatherIcon(String iconType) {
    switch (iconType) {
      case 'rainy':
        return const RainyWeatherIcon(size: 28);
      case 'cloudy':
        return const CloudySunIcon(size: 28);
      default:
        return const SunnyIcon(size: 28);
    }
  }
        ...
        child: Stack(
          children: [
            ...
            GoogleMap(
              ...
              onCameraMove: (pos) {
                if (_isPinMode) setState(() => _currentMapCenter = pos.target);
              },
              markers: {
                              ...
                              children: [
                                SizedBox(
                                  ...
                                  child: _buildWeatherIcon(
                                    _weatherData!['icon'] as String? ?? 'sunny',
                                  ),
                                ),
                            ...
                            children: _searchSuggestions
                                .map(
                                  (s) => InkWell(
                                    borderRadius: BorderRadius.circular(16),
                                    onTap: () =>
                                        _selectSuggestion(s['label'] as String),
                                    child: Padding(
                                      padding: const EdgeInsets.symmetric(
                                        horizontal: 16,
                                        vertical: 12,
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  _zoomBtn(Icons.my_location_rounded, _goToMyLocation),
                ],
              ),
            ),
          ],
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    data['dest_label'] as String? ?? 'Tujuan',
                    style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w700,
                      color: _P.ink2,
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Normal: ${data['normal_time'] as String? ?? '--'}',
                      style: TextStyle(
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  '${factors['traffic_source'] ?? 'Google Maps Traffic'} • Akurasi ${factors['confidence_level'] ?? '--'}',
                  style: const TextStyle(
                    fontSize: 11,
                    color: _P.ink4,
                    fontWeight: FontWeight.w500,
  ...
  Widget _buildAngkotSection() {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        color: _P.bg,
        border: Border(top: BorderSide(color: _P.b100, width: 1)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          if (!_isLoadingAngkot && _angkotLangsung.isNotEmpty)
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemCount: _angkotLangsung.length,
                itemBuilder: (context, i) {
                  final angkot = _angkotLangsung[i];
        ...
        child: Row(
          children: [
            ...
            _nbItem(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const AngkotTrackingScreen()),
              ),
            ),
            _nbFab(),
            _nbItem(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const TrafficHeatmapScreen()),
              ),
            ),
            _nbItem(
              4,
